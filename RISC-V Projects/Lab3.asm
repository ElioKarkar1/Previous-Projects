...
.text
	...
	loop:
	#check if > 1
	bgt a0, zero,outerloop
	#if < 0 ask for user input again and print error msg
	print_str(error_msg)
	print_str(newline)
	print_str(prompt1)
	read_n(t0)
	...
	li t2, 1
	innerloop:
	#if t2 >= t1 go back to outer
	bge t2, t1, increment
	#if rownumber == 1 printstar
	beq t2, t3, printstar
	#if rownumber == last row print star
	beq t1, t0, printstar
	#if rownumber equals the column number print star
	beq t1, t2, printstar
	write_to_buffer(0x20)
	add t2, t2, t3
	j innerloop
	printstar:
	#prints the star adds 1 to t2 and goes back to innerloop
	write_to_buffer(0x2a)
	add t2, t2, t3
	j innerloop
	increment:
	#adds one to t1 and goes back to outerloop
	write_to_buffer(0x2a)
